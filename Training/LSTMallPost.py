...
data1 = pickle.load( open( "11_50_findAnotData.p", "rb" ) )
data2 = pickle.load( open( "51_100_findAnotData.p", "rb" ) )
data3 = pickle.load( open( "101_218_findAnotData.p", "rb" ) )
data4 = pickle.load( open( "218_1000_findAnotData.p", "rb" ) )
featData = data1+data2+data3+data4
...
#Use all annotation for Training
train_sentences = sentences
train_tags = sentence_annotClass
...
posts = pickle.load( open( "../BioNLPtrend/cessation_submissionSentToken_2013-2018.p", "rb" ) )

LSTMpredict = []
for p in posts:#each element is a dictionary
	test_samples = p.values() #list of list of tokenized sentences

	#transform to word id
	test_samples_X = []
	...
	test_samples_X = keras.preprocessing.sequence.pad_sequences(test_samples_X, maxlen=MAX_LENGTH, padding='post')
	#predict
	predictions = model.predict(test_samples_X)
	
	#readable label
	currentSentsLabels = logits_to_tokens(predictions, {i: t for t, i in tag2index.items()})
	print(logits_to_tokens(predictions, {i: t for t, i in tag2index.items()}))
#	sentPostList.append(currentSentence)# list of list of list
	LSTMpredict.append(currentSentsLabels)


pickle.dump(LSTMpredict, open('../BioNLPtrend/cessation_submissionLSTMpredict_2013-2018.p','wb'))
